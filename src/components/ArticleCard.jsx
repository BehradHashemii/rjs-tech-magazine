...
export const ArticleCard = ({ article, variant = 'standard' }) => {
  const getSubcategoryName = (subSlug) => {
    switch (subSlug) {
      case 'ai-articles': return 'هوش مصنوعی';
      case 'internet-network': return 'اینترنت و شبکه';
      case 'security': return 'امنیت و حریم خصوصی';
      case 'space': return 'هوا و فضا';
      case 'engineering': return 'مهندسی';
      case 'fundamental-science': return 'علوم پایه';
      case 'ai-tutorials': return 'آموزش هوش مصنوعی';
      case 'computer-learning': return 'آموزش کامپیوتر';
      case 'mobile-learning': return 'آموزش موبایل';
        ...
        <div className="lg:w-5/12 p-6 lg:p-8 flex flex-col justify-between relative z-10">
          <div>
            <div className="flex items-center gap-3 mb-4">
              <Link 
                to={getSubcategoryLink(article.categorySlug, article.subcategorySlug)}
          ...
          <div>
            <div className="flex items-center justify-between pt-4 border-t border-[#2e3446] text-xs text-slate-400">
              <div className="flex items-center gap-2">
                <span className="text-slate-300 font-medium">{article.author.name}</span>
              </div>
  ...
  if (variant === 'horizontal') {
    return (
      <div className="group bg-[#202432] border border-[#2e3446] hover:border-purple-500/40 rounded-xl p-3 flex gap-4 transition-all hover:bg-[#282d3e]">
        <Link to={`/article/${article.slug}`} className="w-28 sm:w-36 h-24 flex-shrink-0 rounded-lg overflow-hidden relative bg-[#181a20]">
          <img 
            src={article.coverImage} 
            alt={article.title} 
    ...
    <div className="group bg-[#202432] border border-[#2e3446] hover:border-purple-500/40 rounded-2xl overflow-hidden transition-all duration-300 hover:-translate-y-0.5 shadow-md flex flex-col justify-between">
      ...
      <div className="px-5 pb-4 pt-2 border-t border-[#2e3446] flex items-center justify-between text-xs text-slate-400">
        <div className="flex items-center gap-1.5">
          <span className="text-slate-300 text-[11px] font-medium">{article.author.name}</span>
        </div>
